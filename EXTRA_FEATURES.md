...
- Move the mouse vertically to adjust the camera pitch. Pitch is clamped so the camera stays usable and does not flip.
...
- Mouse movement updates camera yaw and pitch continuously, and keyboard movement is frame-rate independent through the per-frame `dt` value.
